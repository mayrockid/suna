...
async def handle_error(
        error: Exception, attempt: int, max_attempts: int
) -> None:
    ...
    try:
        ...
        if max_tokens is not None:
            ...
            if model_name.startswith("bedrock/") and "claude-3-7" in model_name:
                logger.debug(
                    f"Skipping max_tokens for Claude 3.7 model: {model_name}"
                )
        ...
        if model_name.startswith("openrouter/"):
            logger.debug(
                f"Preparing OpenRouter parameters for model: {model_name}"
            )
            ...
            if site_url or app_name:
                ...
                logger.debug(
                    f"Added OpenRouter site URL and app name to headers"
                )

        # Add Bedrock-specific parameters
        if model_name.startswith("bedrock/"):
            logger.debug(
                f"Preparing AWS Bedrock parameters for model: {model_name}"
            )
        ...
        if "claude" in effective_model_name.lower() or "anthropic" in effective_model_name.lower():
            messages = params[
                "messages"]  # Direct reference, modification affects params
                ...
                if isinstance(content, str):
                    ...
                    for item in content:
                        ...
                        if isinstance(item, dict) and item.get(
                                "type"
                        ) == "text" and "cache_control" not in item:
        ...
        if is_anthropic and use_thinking:
            ...
            params[
                "temperature"] = 1.0  # Required by Anthropic when reasoning_effort is used
            logger.info(
...
async def make_llm_api_call(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logger.debug(
                f"Successfully received API response from {model_name}"
            )
            # logger.debug(f"Response: {response}")
            return response

        except (litellm.exceptions.RateLimitError, OpenAIError,
